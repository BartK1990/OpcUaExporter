...
- **Acquisition/** — `SubscriptionAcquisitionEngine` and `PollingAcquisitionEngine` behind `IAcquisitionEngine`, with `AcquisitionCoordinator` starting and rebuilding them as the connection comes and goes. `Bridge:Acquisition:Deadband` (off by default) attaches a server-side `DataChangeFilter`; it triggers on `StatusValue`, never value alone, because a tag going bad must reach the downstream application even when its number has not moved.
...
- **Diagnostics/ProcessCostSampler.cs** — processor time, memory and GC counts for this process, sampled between calls. The dashboard shows it beside the value rate so "the bridge is using CPU" can be told apart from "the bridge is using CPU for nothing" without attaching a profiler to a Windows service.
